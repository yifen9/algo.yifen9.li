...
. You can move to city $v$only if the signal in city $v$is green. (The signal in the city you are moving from can be in either state.)
...
<li>
Select a contiguous subarray from $A$and call it $R_A$.
</li>

<li>
Select a contiguous subarray from $B$of the same length as $R_A$and call it $R_B$.
</li>

<li>
Overwrite the contents of $R_B$with the contents of $R_A$.
</li>
...
up to $10^5$times in any order. See the "Sample Output" section below for specific examples of signal operation and movement.
...
<p>
If the trip is unsuccessful, or if there are invalid signal operations/movements, or if the number of signal operations and movements exceeds $10^5$, the result is judged as 
<span>
WA
</span>
.
...
<span>
\( \text{round}(10^9 \times \left(\frac{\rm{minimum\ absolute\ score\ among\ all\ participants}}{\rm{your\ absolute\ score}}\right)) \)
</span>
relative evaluation score is obtained, and the sum of the relative evaluation scores is the score of the submission.            
...
<b>
minimum absolute score among all participants
</b>
...
<li>
We will publish <a href="https://img.atcoder.jp/ahc036/seeds.txt">seeds.txt</a>(sha256=`607aca150bd5f8f062937b02e6d14c15f7661aefbd279f150043e6d7e47f8d3c`) after the contest is over.
</li>
...
Only the last submissions are used to calculate the 
<b>
minimum absolute score among all participants
</b>
for each test case when calculating the relative evaluation scores.
...
If the length of $R_A$and $R_B$is $l (1 \le l \le L_B)$, 
                            ...
                            then output $l, P_A, P_B$separated by spaces following the leading `s`.
...
If the destination city is $v$,
                            then output $v$separated by spaces following the leading `m`.
...
<p>
<a href="https://img.atcoder.jp/ahc036/e5f02df53f30d36e.html?lang=en&output=sample">Show example</a>
</p>
...
<li>
Generate $x_i = \mathrm{rand}(0,1000), y_i = \mathrm{rand}(0,1000)$.
</li>

<li>
Continue to choose $(x_i, y_i)$until there are no vertices in $V$that have a distance of less than $D_{vmin}$to $(x_i, y_i)$.
</li>
...
<li>
List all vertex pairs within a distance of $D_{emax} / 2$, shuffle them randomly, and add each pair to $E$if they do not intersect with any already added edges. Here, for edges to intersect means that if vertex $i$is placed at coordinates $(x_i, y_i)$and edges are drawn as line segments connecting their endpoints, two edges share a common point other than their endpoints.
</li>

<li>
List all vertex pairs with a distance greater than $D_{emax} / 2$but less than $D_{emax}$, shuffle them randomly, and add each pair to $E$if they do not intersect with any already added edges.
</li>
...
<li>
<a href="https://img.atcoder.jp/ahc036/e5f02df53f30d36e.html?lang=en">Web Visualizer / Input Generator</a>: For displaying animated visualizations.
</li>

<li>
<a href="https://img.atcoder.jp/ahc036/e5f02df53f30d36e.zip">Local Tester / Visualizer / Input Generator</a>: Please set up a <a href="https://www.rust-lang.org">Rust language</a>build environment to use this.
                            
<ul>

<li>
<a href="https://img.atcoder.jp/ahc036/e5f02df53f30d36e_windows.zip">Pre-compiled binary for Windows</a>: If you have troubles setting up Rust environment, use this.
</li>
...
<li>
<a href="https://img.atcoder.jp/ahc036/e5f02df53f30d36e_sample.zip">Sample code (C++, Python)</a>: Sample answers. Implemented as follows:
